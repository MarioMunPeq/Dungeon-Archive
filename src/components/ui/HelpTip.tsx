  ...
  return (
    <span ref={containerRef} className={cn("relative inline-flex shrink-0", className)}>
      <button
        ...
        aria-label={label}
        aria-expanded={open}
        className="hitbox-expand inline-flex h-5 w-5 select-none items-center justify-center rounded-full bg-info/15 text-xs font-bold text-info transition-all duration-150 active:scale-90"
      >
        ?
      </button>
      {open && (
